# Notices

ICT Branch Toolkit is licensed under the GNU Affero General Public License v3.0.

**Originally developed by the Texas Communications Unit (TX-COMU).**

The ICT Branch Toolkit name identifies this open-source software project. The TX-COMU name and logo are not licensed for use in a way that implies sponsorship, certification, or endorsement of a third-party installation or modified version.

References to FEMA, CISA, NIFOG, ICS forms, or TAK do not imply endorsement by those organizations or programs. Third-party dependency notices are available from the corresponding package metadata and licenses.

Production frontend builds reproduce the complete MapLibre GL JS license and
bundled third-party notices at
`/third-party/maplibre-gl-LICENSE.txt`.

