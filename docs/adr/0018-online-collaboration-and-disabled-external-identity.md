...
Presence contains only display name, section, viewing/editing state, device
identifier, sequence, and lease timestamps. It does not contain form values,
...
or an explicit `[REDACTED]` marker.

The provisional default preserves the existing plan-authority boundary:
Administrator, COML, and COMC may view and edit restricted contact fields.
COMT, Contributor, Read-only, and every new role fail closed unless an approved
incident rule grants access. The official ICS-205 PDF continues to omit these
contact fields.
...
implemented. Configuration cannot turn the disabled provider into a live
connection.
...
notification, all derived-output redaction, deployment concurrency, and live
