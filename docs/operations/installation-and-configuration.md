...
| `ICT_TERRAIN_PROVIDER`                       | disabled provider       | Server-selected P3.1 profile adapter. Keep disabled until the exact dataset, adapter, terms, privacy, transformation, limits, and supported use pass the [terrain approval procedure](terrain-analysis.md). |
| `ICT_TERRAIN_ENGINE`                         | provisional sampled LOS | Server-selected, versioned analysis method. It cannot be chosen by the browser. A changed engine/version requires a new exact approval.                                                                     |
| `ICT_APPROVED_TERRAIN_CONFIGURATIONS`        | `[]`                    | Exact provider/provider-version/dataset-product/dataset-version/source-digest/engine/engine-version objects. Empty fails closed. Never include credentials.                                                 |
| `ICT_SYNTHETIC_TERRAIN_MODE`                 | `flat`                  | Deterministic fixture mode only. It never represents actual terrain and has no effect unless the synthetic provider is explicitly selected and approved.                                                    |
| `ICT_TERRAIN_MAX_DISTANCE_M`                 | `200000`                | Defensive profile-distance ceiling, constrained to 1,000–500,000 meters. It is not an accuracy or capacity rating.                                                                                          |
| `ICT_TERRAIN_MAX_SAMPLES`                    | `1001`                  | Defensive sample-count ceiling, constrained to 2–5,001 samples. It is not a safe concurrency or dataset-capacity rating.                                                                                    |
| `ICT_OFFLINE_ENABLED`                        | `false`                 | Enables only the P3.2 controlled package contract. Keep disabled until the [offline human gate](offline-and-intermittent-operation.md) is recorded.                                                         |
| `ICT_OFFLINE_APPROVED_FOR_NON_SYNTHETIC_USE` | `false`                 | Deployment assertion for the separate security/privacy/records/operations gate. It does not grant access or override data classification.                                                                   |
| `ICT_OFFLINE_MAX_PACKAGE_BYTES`              | `5242880`               | Canonical server-package byte ceiling, constrained to 65,536–52,428,800 bytes. Changes require device, database, and security evidence.                                                                     |
| `ICT_OFFLINE_MAX_QUEUE_ITEMS`                | `500`                   | Ordered mutation batch ceiling, constrained to 1–5,000. It is not a tested throughput or concurrency rating.                                                                                                |
| `ICT_OFFLINE_DEFAULT_TTL_HOURS`              | `24`                    | Default package expiration. It must not exceed the configured maximum or adopted retention rule.                                                                                                            |
| `ICT_OFFLINE_MAX_TTL_HOURS`                  | `72`                    | Maximum selectable expiration, constrained to 720 hours. Raising it increases device-loss and stale-access exposure.                                                                                        |
| `ICT_OFFLINE_CLOCK_SKEW_SECONDS`             | `300`                   | Maximum client/server clock difference accepted at synchronization, constrained to 0–86,400 seconds.                                                                                                        |
...
14. Terrain analysis reports disabled unless the exact provider, dataset, and
    engine approval exists. If a synthetic evaluation is approved, verify
...
16. Offline capability reports disabled unless its exact scope and deployment
    gate are approved. For a synthetic evaluation, verify explicit selection,
    encryption/wrong-passphrase behavior, network loss/reconnect, queue
    integrity, duplicate/stale/revoked handling, conflict decisions,
    expiration, lock/purge, safe cache clearing, and minimized support export
    through the
    [offline operations guide](offline-and-intermittent-operation.md).
