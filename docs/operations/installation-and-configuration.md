...
| `ICT_INVENTORY_ENCRYPTION_KEY`                                                     | URL-safe base64 encoding of exactly 32 random bytes. Protects accountable-checkout identifiers at the application layer. Back it up as a secret; losing or replacing it makes retained values unreadable. |
...
| `ICT_TOKEN_TTL_SECONDS`                   | `43200`                 | Maximum local-token lifetime in seconds. Must be greater than zero. Shortening it can immediately expire existing sessions.                                                                                 |
